...
- [VPS migration 9/22-23](project_vps_migration_sep22.md) — cutover to new 6-core/11GB host; portfolio_sync bug fixed (IBKR); Danelfin confirmed inactive; overnight: root-caused+fixed AlpacaBroker's enum-status bug (root cause of sleeved reconciliation break) + built real per-sleeve fill correction + re-verified IB Gateway exposure as already-mitigated
- [SSH hardening preference](feedback_ssh_hardening_preference.md) — user wants to keep SSH password auth; don't propose disabling it even for a real audit finding without asking first
- [Planning cycle feature 9/23](project_planning_cycle_feature.md) — LIVE on both IBKR + Alpaca (09:15 ET daily) since 9/23; first real scheduled fire is 9/24 morning, worth checking then
- [Reconciliation audit 9/23](project_reconciliation_audit_sep23.md) — IBKR + Alpaca wash-trade fixes deployed+verified; sleeve drift write-off made things WORSE, rolled back — legacy drift now accepted debt, don't retry that approach
- [Exhaustive audit closure](feedback_exhaustive_audit_closure.md) — do one full parallel sweep when checking system health, don't trickle findings one round at a time

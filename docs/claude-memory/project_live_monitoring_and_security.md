...
- Access going forward: `https://109.207.76.82/` (or whatever the VPS's current public IP is — re-check, don't trust this blindly next session) with the `admin` credentials (password was shown to the user once in-chat; check with them or a password manager rather than assuming it's still `ShFnd5MWNkoA586ScWBrt94v` — rotate if there's any doubt it leaked).
